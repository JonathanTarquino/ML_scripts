...
def extract3DFeatureInfo(img_org,mask_org,ws_options=[3,5,7,9,11],class_options = ['raw','gray','gradient','haralick','gabor','laws','collage']):
  ...
  for ws in ws_options:
    ...
    coll = compute_collage3d(img_org,mask_complete, haralick_windows=ws)
    end = time.time()
    print('Elapsed time for Collage:',end-start )

    feat_vect = extract3DFeatIntensities(coll[1], mask_org)
    print(coll[2],feat_vect)
